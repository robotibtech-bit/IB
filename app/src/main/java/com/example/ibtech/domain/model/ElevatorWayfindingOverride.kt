 ...
 * 매핑이다. "4열람실(스마트).png"처럼 안내도 파일명에 붙은 괄호는 [StairsWayfindingOverride]의
 * "3열람실(스마트)" 사례처럼 실제 POI 이름에는 없을 가능성이 높아 미리 뗀 값을 키로 썼다 —
 * 다만 아래 목록 중 "디지털자료실"을 제외한 나머지는 아직 이 로봇에 동기화된 POI 목록에 없어
 * (2026-08-13 기준) 실제 이름을 로봇에서 직접 확인하지 못했다. 새 POI가 동기화된 뒤 관리자
 * 화면에서 표시명이 아래 키와 다르면 그 값으로 고쳐야 한다.
 */
