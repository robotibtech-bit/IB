...
object DefaultFacilityContent {
    ...
    fun build(): List<Facility> {
        ...
        val corridor = WayfindingCorridorOverride.facilityIds().map { name ->
            defaultFacility(name, floor = 1, sortOrder = next())
        }
        // strings.xml 이용시간표에 명시된 1층 시설.
        val kidsRoom = listOf(defaultFacility("어린이자료실", floor = 1, sortOrder = next()))
        // 계단 대상 — 실제 로봇 이동은 연결통로 경유(사용자 확인, 2026-08-18).
        val stairs = StairsWayfindingOverride.facilityIds().map { name ->
            defaultFacility(name, floor = KNOWN_FLOOR_BY_NAME[name] ?: Facility.UNSET_FLOOR, sortOrder = next())
        }
    ...
    /** [SyncPoiUseCase]가 신규 POI에 쓰는 기본값(안내 방식 동행+위치, 방향 정면, 노출 켬)과
     * 맞춰 둔다 — 디폴트로 등록된 시설과 나중에 새로 동기화되는 시설이 다르게 보이지 않도록.
    ...
    private fun defaultFacility(name: String, floor: Int, sortOrder: Int) = Facility(
        id = name,
        sourcePoiName = name,
        name = name,
        floor = floor,
